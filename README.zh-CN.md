...
用于控制 X5-2023 外置夹爪上的 DaMiao DM-J4310 电机。克隆本仓库后，可在 Linux 上通过 SocketCAN 独立控制夹爪，或与 X5 机械臂做联合控制。

- **独立夹爪控制**：只通过 SocketCAN 控制已配置的夹爪 ESC，不初始化机械臂。
- **机械臂与夹爪联合控制**：加载仓库内置厂家运行库，在同一 `can2` 接口上同时控制 X5 机械臂和外置夹爪。

功能包括当前位置保持、相对微动、双端点软件标定、归一化开度控制、全开/全闭线性控制、交互式键盘控制、反馈读取，以及超时、超速、越界、反向运动和控制器冲突保护。

> **重要：** SDK 永远不会发送电机 `SET_ZERO`。标定只记录软件端点，不改电机内部零点。独立夹爪动作正常结束或异常退出时都会发送 `DISABLE`。

仓库地址：<https://github.com/zld123850-bit/x5_gripper_sdk>
...
实机还需要外置 DM-J4310 夹爪；联合模式同时需要 X5-2023 机械臂。

只有独立夹爪控制使用纯 Python SocketCAN。机械臂联合模式会加载包内的 CPython 3.12 x86_64 厂家二进制扩展，必须使用兼容的 Python 和平台。

本仓库不负责创建或配置 SocketCAN 接口。请先按 USB-CAN 适配器的流程把接口设为 `UP`。
...
如果已有 Conda 环境，也可以先激活再安装：

```bash
conda activate <your-python312-env>
python -m pip install -e .
```
...
启动脚本按自身所在目录定位源码和标定文件，可在任意工作目录执行；下面示例都假设当前目录是仓库根目录。
...
4. 核对夹爪电机 CAN ID。仓库默认使用 `8`，未确认实机 ID 时不要直接运行。
5. 确认 `kp=5.0`、`kd=0.2` 适用于当前设备；它们是本项目使用的受限参数，不是所有设备的通用值。
...
`--acknowledge-unverified-hardware` 表示操作者已经核对 CAN ID、固定机械臂并准备好急停，不是跳过 SDK 内部安全检查。程序会尝试独占目标电机、确认反馈静止、短暂保持，然后自动失能。若运动方向、速度、声音或温度异常，请立即按物理急停，不要继续标定。
...
每个命令输入一个字母后按 Enter。动作返回 `target_reached=false` 时，不要继续重复同方向命令；先检查机械端点、障碍物、传动卡滞和反馈力矩。
...
当前控制模型要求张开时编码器值减小，即 `open_position_rad < closed_position_rad`。方向不符合时程序会拒绝保存或拒绝运动。
...
脚本使用自身位置定位源码，默认将结果写入同目录的 `gripper-calibration.json`，不会初始化机械臂。它会传入 `--ignore-existing-calibration`，因此重新标定时不会使用旧端点保护。

按住 `c` / `o` 期间会持续发送力矩运动命令，松开立即停止并失能。单段行程上限为闭合 `0.20 rad`、张开 `0.50 rad`；仍按住则自动接下一段。靠近机械端点时请点按或减小 `X5_GRIPPER_CALIBRATION_STEP_RAD`。
...
1. 按住 `c` 连续移动到闭合端，松键后按 `z`。
2. 按住 `o` 连续移动到最大安全张开端，松键后按 `m`。
...
`calibrate` 本身不会发送位置或力矩运动目标，只采集静止反馈；若失能后没有反馈，SDK 可能短暂发送 `ENABLE` 以读取状态，并在采集后再次发送 `DISABLE`。程序会先要求在闭合端输入大写 `CLOSED`，再要求在张开端输入大写 `OPEN`。

默认安全硬限为单次闭合不超过 `0.20 rad`、单次张开不超过 `0.50 rad`、前馈力矩绝对值不超过 `0.50 Nm`。标定前没有软件端点保护，不要使用一次命令尝试完整行程。

### 标定文件注意事项

仓库自带的 `gripper-calibration.json` 是序列号 `X5-2023-001` 的设备标定，不是通用出厂参数。更换夹爪、电机、机械传动、CAN 接口、设备序列号，或编码器坐标发生变化后，必须重新标定，不能直接复用该文件或交换两个端点绕过方向检查。

标定文件会校验模型、电机类型、设备序列号、CAN 接口、CAN ID、编码范围和行程。保存采用原子替换，不会发送 `SET_ZERO`。
...
脚本默认加载同目录的 `gripper-calibration.json`，并自动设置源码路径和厂家动态库路径。联合模式只允许 `interface=can2`、`motor-can-id=8`，机械臂与夹爪共用 CAN 总线，运行期间不能再启动其他机械臂或夹爪控制程序。

阅读启动提示并输入 `1` 后，程序会在厂家初始化前将夹爪低速移动到标定参考位置，然后完成受监控的控制权交接。按键无需 Enter：
...
每次机械臂方向键请求末端移动 `1 cm`，目标轨迹速度为 `1 cm/s`。普通终端没有真实 KeyUp 事件，夹爪的持续开合依赖键盘自动重复；若桌面环境关闭了自动重复，按住按键只会产生一次很小的目标增量。

联合控制不会同时运行直连 `X5Gripper` 和厂家 `SingleArm`。原始标定 JSON 以只读方式加载，联合模式中的 `Z/M` 标定写入被禁用；需要重新标定时请使用上面的独立标定流程。
...
长期默认值集中在 `start-calibrated-gripper.sh` 顶部的“常用参数区”。常用环境变量：
...
还可设置 `X5_GRIPPER_FEEDBACK_CAN_ID`、`X5_GRIPPER_KP`、`X5_GRIPPER_KD`、`X5_GRIPPER_NUDGE_STEP_RAD`、`X5_GRIPPER_NUDGE_TORQUE_NM`、`X5_GRIPPER_HOLD_SECONDS`、`X5_GRIPPER_ARM_SDK_PATH`、`X5_GRIPPER_ARM_MOTION_TIMEOUT` 和 `X5_GRIPPER_ARM_MAX_VELOCITY`。
...
`MotionResult` 包含起点、目标、最终状态、最大速度、最大位移、最大跟踪误差、采样数、`target_reached` 和 `stopped_by_request`。完整最小示例见 [`examples/basic_control.py`](examples/basic_control.py)。
...
- `feedback_can_id=None` 会按反馈 payload 内的电机 ID 匹配；确认反馈 CAN ID 后，建议配置精确值。
...
start-gripper-calibration.sh                 独立夹爪标定启动脚本
start-calibrated-gripper.sh                  已标定夹爪 / 联合控制启动脚本
```
...
本项目按 MIT License 发布，详见 [`pyproject.toml`](pyproject.toml)。
